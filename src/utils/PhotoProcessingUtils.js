...
/**
 * Check if photo matches extension filter
 * @param {Object} photo - Photo object
 * @param {string} extensionFilter - Extension filter string
 * @returns {boolean} Whether photo matches filter
 */
export function matchesExtensionFilter(photo, extensionFilter) {
...
/**
 * Check if photo has required star rating
 * @param {Object} photo - Photo object
 * @param {number} starFilter - Minimum star rating
 * @returns {boolean} Whether photo meets star requirement
 */
export function meetsStarRequirement(photo, starFilter) {
    if (starFilter <= 0) return true;
    return photo.star && photo.star >= starFilter;
}

/**
 * Check if photo has comment (when comment filter is active)
 * @param {Object} photo - Photo object
 * @param {boolean} hasCommentFilter - Whether comment filter is active
 * @returns {boolean} Whether photo meets comment requirement
 */
export function meetsCommentRequirement(photo, hasCommentFilter) {
    if (!hasCommentFilter) return true;
    return photo.comment && photo.comment.trim() !== "";
}

/**
 * Check if photo has tags (when tag filter is active)
 * @param {Object} photo - Photo object
 * @param {boolean} hasTagFilter - Whether tag filter is active
 * @returns {boolean} Whether photo meets tag requirement
 */
export function meetsTagRequirement(photo, hasTagFilter) {
