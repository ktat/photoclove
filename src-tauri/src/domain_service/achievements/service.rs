...
use super::definitions::{
    self, AchievementCategory, AchievementDefinition, ACHIEVEMENTS, DAYS_ACHIEVEMENTS,
    PHOTO_COUNT_ACHIEVEMENTS, STAR_COUNT_ACHIEVEMENTS,
};
...
impl AchievementService {
    ...
    pub fn get_all_achievements(&self) -> Result<Vec<AchievementWithProgress>, String> {
        ...
        for def in ACHIEVEMENTS {
            ...
            result.push(AchievementWithProgress::from_def_and_progress(
                def, progress,
            ));
        }
    ...
    pub fn check_first_action(
        &self,
        achievement_id: &str,
    ) -> Result<AchievementCheckResult, String> {
        let newly_achieved = self
            .db
            .mark_achievement_achieved(achievement_id)
    ...
    pub fn check_photo_count(&self) -> Result<AchievementCheckResult, String> {
        ...
        for achievement_id in PHOTO_COUNT_ACHIEVEMENTS {
            if let Some(def) = definitions::get_achievement_def(achievement_id) {
                let was_newly_achieved = self
                    .db
                    .upsert_achievement(achievement_id, count, def.threshold)
    ...
    pub fn check_monthly_achievements(&self) -> Result<AchievementCheckResult, String> {
        ...
        if months.len() >= 12 {
            let was_newly_achieved = self
                .db
                .upsert_achievement("monthly_all", 12, 12)
                .map_err(|e| format!("Failed to upsert achievement: {}", e))?;
            ...
            let _ = self
                .db
                .upsert_achievement("monthly_all", months.len() as i64, 12);
        }
    ...
    pub fn check_days_achievements(&self) -> Result<AchievementCheckResult, String> {
        ...
        for achievement_id in DAYS_ACHIEVEMENTS {
            if let Some(def) = definitions::get_achievement_def(achievement_id) {
                let was_newly_achieved = self
                    .db
                    .upsert_achievement(achievement_id, unique_days, def.threshold)
                    .map_err(|e| format!("Failed to upsert achievement: {}", e))?;
        ...
        if has_leap_day {
            let was_newly_achieved = self
                .db
                .mark_achievement_achieved("leap_year")
                .map_err(|e| format!("Failed to mark achievement: {}", e))?;
        ...
        let unique_month_days: i64 = conn
            .query_row(
        ...
        if let Some(def) = definitions::get_achievement_def("all_dates_complete") {
            let was_newly_achieved = self
                .db
                .upsert_achievement("all_dates_complete", unique_month_days, def.threshold)
                .map_err(|e| format!("Failed to upsert achievement: {}", e))?;

            if was_newly_achieved {
                let progress = self
                    .db
                    .get_achievement("all_dates_complete")
                    .map_err(|e| format!("Failed to get achievement: {}", e))?;
                newly_achieved.push(AchievementWithProgress::from_def_and_progress(
                    def,
                    progress.as_ref(),
    ...
    pub fn check_special_achievements(&self) -> Result<AchievementCheckResult, String> {
        ...
        if let Some(def) = definitions::get_achievement_def("night_owl") {
            if let Ok(true) = self
                .db
                .upsert_achievement("night_owl", night_photos, def.threshold)
            {
        ...
        if let Some(def) = definitions::get_achievement_def("early_bird") {
            if let Ok(true) = self
                .db
                .upsert_achievement("early_bird", early_photos, def.threshold)
            {
        ...
        let golden_hour_photos: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM photo_metadata
             WHERE (delete_flg = 0 OR delete_flg IS NULL)
        ...
        if let Some(def) = definitions::get_achievement_def("golden_hour") {
            if let Ok(true) =
                self.db
                    .upsert_achievement("golden_hour", golden_hour_photos, def.threshold)
            {
        ...
        if let Some(def) = definitions::get_achievement_def("gear_collector") {
            if let Ok(true) =
                self.db
                    .upsert_achievement("gear_collector", camera_count, def.threshold)
            {
        ...
        if let Some(def) = definitions::get_achievement_def("tag_master") {
            if let Ok(true) = self
                .db
                .upsert_achievement("tag_master", tag_count, def.threshold)
            {
        ...
        if let Some(def) = definitions::get_achievement_def("album_curator") {
            if let Ok(true) =
                self.db
                    .upsert_achievement("album_curator", album_count, def.threshold)
            {
    ...
    pub fn check_all_achievements(&self) -> Result<AchievementCheckResult, String> {
        ...
        Ok(AchievementCheckResult {
            newly_achieved: all_newly_achieved,
        })
    }
}
