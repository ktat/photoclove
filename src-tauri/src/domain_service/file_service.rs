...
pub fn restore_from_trash(
    ...
    let trash_file_path = trash_path.join(&parent_path).join(file_name);

    // Restore to original location (target_file.parent() already contains the full path)
    let restore_dir = target_file.parent().ok_or_else(|| {
        std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            format!("Cannot get parent directory for restore: {:?}", target_file),
        )
    })?;
