                    ...
                    match serde_json::from_str::<serde_json::Value>(&data) {
                        Ok(json) => {
                            ...
                            if let Some(access_token) = json.get("access_token").and_then(|v| v.as_str()) {
                                println!("✅ Has access_token field ({} chars)", access_token.len());
                            ...
                            if let Some(refresh_token) = json.get("refresh_token").and_then(|v| v.as_str()) {
                                println!("✅ Has refresh_token field ({} chars)", refresh_token.len());
                            ...
                            // Check expiration
                            if let Some(expires_at) = json.get("expires_at").and_then(|v| v.as_str()) {
                                ...
                                match DateTime::parse_from_rfc3339(expires_at) {
                                    Ok(exp_time) => {
                                        ...
                                        if duration.num_seconds() > 0 {
                                            println!("   ⏱️  Valid for: {} minutes", duration.num_minutes());
                                        } else {
                                            println!("   ⚠️  EXPIRED {} minutes ago", -duration.num_minutes());
                                        }
                                    }
                        ...
                        Err(e) => {
                            ...
                            println!("Raw data preview: {}", &data.chars().take(50).collect::<String>());
                        }
                    }
                }
